  ...
  useEffect(() => {
    ...
    const filteredPoints = points.filter(
      (point, index, allPoints) =>
        allPoints.findIndex((candidate) => candidate.lat === point.lat && candidate.lng === point.lng) ===
        index,
    )
    ...
    globeRef.current
      ...
      .arcAltitude((d: object) => (d as { arcAlt: number }).arcAlt)
      .arcStroke(() => [0.32, 0.28, 0.3][Math.round(Math.random() * 2)])
      .arcDashLength(defaultProps.arcLength)
      .arcDashInitialGap((d: object) => (d as { order: number }).order)
      .arcDashGap(15)
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const ringsData = data
        .filter((_, index) => newNumbersOfRings.includes(index))
        .map((item) => ({
          lat: item.startLat,
          lng: item.startLng,
          color: item.color,
...
export function World(props: WorldProps) {
  ...
  const camera = useMemo(() => {
    ...
    c.position.z = cameraZ
    return c
  }, [])

  return (
    <Canvas scene={scene} camera={camera} resize={{ scroll: false }}>
      <WebGLRendererConfig />
